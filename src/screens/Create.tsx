...
export default function Create({ onClose }: { onClose: () => void }) {
  ...
  const audienceFromPrivacy = (): 'PUBLIC' | 'FOLLOWERS' | 'GROUP' => {
    if (selectedGroupId) return 'GROUP'
    if (privacy === 'Amigos') return 'FOLLOWERS'
    return 'PUBLIC'
  }

  const publishPost = async () => {
    if (!canShare || publishing) return
    setPublishing(true)
    setPublishError(null)
    try {
      const mediaUrls: string[] = []
      if (photoFile) {
        mediaUrls.push(await uploadMedia(photoFile))
      }

      const taggedHandles = contacts
        .filter((c) => taggedIds.includes(c.id))
        .map((c) => c.handle.replace(/^@/, ''))

      await createPost({
        body: caption.trim(),
        audience: audienceFromPrivacy(),
        groupId: selectedGroupId,
        mediaUrls,
        tags: extractTags(caption),
        mentionHandles: [...extractMentions(caption), ...taggedHandles],
      })
      onClose()
    } catch (err) {
  ...
  const publishStory = async () => {
    if (!photoFile || publishing) return
    setPublishing(true)
    setPublishError(null)
    try {
      const url = await uploadMedia(photoFile)
      await createStory(url, photoFile.type.startsWith('video/') ? 'VIDEO' : 'IMAGE')
      onClose()
    } catch (err) {
      setPublishError(err instanceof Error ? err.message : 'Não foi possível publicar o story.')
    } finally {
      setPublishing(false)
    }
  }
      ...
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm">
        <div className="w-full max-w-[540px] rounded-t-[22px] sm:rounded-[22px] bg-[#f6f7fb] p-6 shadow-2xl">
          ...
          <button
            ...
            disabled={!photoFile || publishing}
            className={`mt-4 w-full rounded-[12px] py-3.5 text-[14px] font-semibold text-[#0d0f13] transition-opacity ${photoFile && !publishing ? 'opacity-100' : 'opacity-40'}`}
            style={{ background: '#e7fe8e' }}
          >
            {publishing ? 'Publicando…' : 'Publicar story'}
          </button>
          {publishError && mode === 'story' && (
            <p className="mt-2 text-center text-[13px] text-red-600">{publishError}</p>
          )}
        </div>
      </div>
    )
        ...
        <div className="px-6 pb-6 space-y-4">
          ...
          <div className="rounded-[16px] overflow-hidden bg-neutral-50 border border-neutral-200">
            ...
            >
              ...
              <span className="text-[13px] text-[#6c7186] truncate max-w-[140px]">{selectedGroup?.name || 'Nenhum'}</span>
              <ChevronRight size={16} color="#6c7186" />
            </button>
            {showGroupPicker && (
              <div>
                ...
                >
                  ...
                  {!selectedGroupId && <Check size={16} color="#004E44" />}
                </button>
                {groups.map((g) => (
                  <button
                    key={g.id}
                  ...
                  >
                    ...
                    {selectedGroupId === g.id && <Check size={16} color="#004E44" />}
                  </button>
                ))}
              </div>
            )}
          ...
          {publishError && (
            <p className="text-center text-[13px] font-medium text-red-600">{publishError}</p>
          )}
          ...
          <button
            ...
            className={`w-full rounded-[14px] py-3.5 text-[15px] font-bold text-[#0d0f13] transition-opacity ${canShare && !publishing ? 'opacity-100' : 'opacity-40'}`}
